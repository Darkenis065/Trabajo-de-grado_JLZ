...
The action in physics is defined in the next way \cite{Landau}:
...
\begin{equation}
    \delta S = \int_{t_1}^{t_2} \mathcal{L}(q + \delta q,\dot{q} + \delta \dot{q},t)dt  
\end{equation}

For the next step is necesary to do some asumptions, one of them is that at the boundaries our system satisfy $q(t_1) =  q_1 \& q(t_2) = q_2$, we also had to define $q(t,\alpha)$ as:
\newpage
\begin{align}
